...
body, artifacts = run_channel_budget_repair_full_chain_v124(root, bundle)
...
for name in ("assemblies", "screens", "dags", "dynamics", "materials")
    inventory["v124_$(name)"] = write_stream(joinpath(output, "v124_$(name).jsonl"),
        artifacts["v115_streams"][name])
end
inventory["v124_conservation"] = write_stream(joinpath(output,
    "v124_conservation.jsonl"), artifacts["v116_rows"])
inventory["v124_channels"] = write_stream(joinpath(output,
    "v124_channels.jsonl"), artifacts["v117_rows"])
...
report = """# v124 channel-budget repaired full-chain acceptance
