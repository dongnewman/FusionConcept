...
body, artifacts = run_thermal_cycle_repair_full_chain_v122(root, bundle)
...
for name in ("assemblies", "screens", "dags", "dynamics", "materials")
    inventory["v122_$(name)"] = write_stream(joinpath(output, "v122_$(name).jsonl"),
        artifacts["v115_streams"][name])
end
inventory["v122_conservation"] = write_stream(joinpath(output,
    "v122_conservation.jsonl"), artifacts["v116_rows"])
inventory["v122_channels"] = write_stream(joinpath(output,
    "v122_channels.jsonl"), artifacts["v117_rows"])
...
report = """# v122 thermal-cycle repaired full-chain acceptance
