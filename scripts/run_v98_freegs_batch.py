#!/usr/bin/env python3
"""Parallel fail-closed FreeGS verification of all v98 computational candidates."""
...
def run_one(job: tuple[dict, str, str, str]) -> dict:
    ...
    return {"request_index": candidate["request_index"], "status": "error",
            "failed_gates": ["runner_error"], "result_hash": None,
            "process_exit_code": process.returncode,
...
def main() -> int:
    ...
    runner = Path(__file__).with_name("run_v98_freegs_candidate.py").resolve()
    candidates = [json.loads(line) for line in source.read_text(
        encoding="utf-8").splitlines() if line.strip()]
    jobs = [(candidate, str(inputs / f"candidate_{candidate['request_index']:07d}.json"),
             str(results / f"freegs_{candidate['request_index']:07d}.json"), str(runner))
    ...
    artifact = {
        ...
        "protocol_id": "fusionconceptai-v98-freegs-batch-v3-20260829",
        ...
        "claim_boundary": (
            "Candidate-bound FreeGS equilibrium verification only. Subset passes cannot be "
            "promoted to whole-device, complete-stability, engineering, or validation pass."
        ),
    }
